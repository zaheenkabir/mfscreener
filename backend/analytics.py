...
def download_benchmark(ticker="NIFTY 50", period="max"):
    ...
    ticker: Can be NSE name (e.g., 'NIFTY 50') or YF symbol (e.g., '^NSEI', 'GOLDBEES.NS')
    """
    # Imports moved inside function as per instruction, with correction for yfinance
    import pandas as pd
    import numpy as np
    import yfinance as yf
    from datetime import date, timedelta # Changed datetime to timedelta
    ...
    # Multi-ticker fallback for Debt proxy
    if ticker == "NIFTY 10 YR BENCHMARK G-SEC":
        for t in ["SETF10GILT.NS", "ICICIGILT.NS", "LICNETFGILT.NS", "LIQUIDBEES.NS"]:
             try:
                yf_period_map = {"max": "max", "10Y": "10y", "5Y": "5y", "3Y": "3y", "1Y": "1y"}
                data = yf.download(t, period=yf_period_map.get(period, "max"), interval="1d", progress=False)
                if data is not None and not data.empty:
                    ...
                    # Basic validity check - need recent data
                    if not series.empty and series.index[-1] > (pd.Timestamp.now() - pd.Timedelta(days=30)):
                        return series
             except Exception as e:
                print(f"Failed to fetch {t}: {e}")
                continue
    
    # Check if it's an ETF (use yfinance directly)
    if ticker.upper() in ["GOLD", "SILVER"] or ticker.endswith(".NS"):
        yf_symbol = ticker_map_yf.get(ticker.upper(), ticker)
        try:
            yf_period_map = {"max": "max", "10Y": "10y", "5Y": "5y", "3Y": "3y", "1Y": "1y"}
            data = yf.download(yf_symbol, period=yf_period_map.get(period, "max"), interval="1d", progress=False)
            if data is not None and not data.empty:
                if isinstance(data.columns, pd.MultiIndex):
    ...
    # PRIMARY: Use nsepython for NSE indices
    nse_symbol = ticker_map_nse.get(ticker.upper(), ticker.upper())
    
    try:
        end_date = date.today().strftime("%d-%b-%Y")
        years_back = 15
        if period == "10Y": years_back = 10
        elif period == "5Y": years_back = 5
        elif period == "3Y": years_back = 3
        elif period == "1Y": years_back = 1
        
        start_date = (date.today() - pd.DateOffset(years=years_back)).strftime("%d-%b-%Y")
        
        data = index_history(nse_symbol, start_date, end_date)
        
        if data is not None and not data.empty:
            if 'HistoricalDate' not in data.columns and 'Date' in data.columns:
                data['HistoricalDate'] = data['Date']
                
            data['Date'] = pd.to_datetime(data['HistoricalDate'], format='mixed')
            data = data.set_index('Date').sort_index()
            
            close_col = 'CLOSE' if 'CLOSE' in data.columns else ('Close' if 'Close' in data.columns else ('Index Value' if 'Index Value' in data.columns else None))
            if close_col:
                series = pd.to_numeric(data[close_col], errors='coerce').dropna()
                series.index = pd.to_datetime(series.index).normalize()
                return series[~series.index.duplicated(keep='first')].sort_index()
    except Exception as e:
        print(f"nsepython error for {nse_symbol}: {e}")
    
    # FALLBACK: Use yfinance
    yf_symbol = ticker_map_yf.get(ticker.upper(), "^NSEI")
    try:
        yf_period_map = {"max": "max", "10Y": "10y", "5Y": "5y", "3Y": "3y", "1Y": "1y"}
        data = yf.download(yf_symbol, period=yf_period_map.get(period, "max"), interval="1d", progress=False)
        if data is not None and not data.empty:
        ...
        print(f"yfinance fallback error for {yf_symbol}: {e}")
