...
import importlib
import screener_app.utils
importlib.reload(screener_app.utils)
...
from screener_app.utils import (
    ...
    get_sebi_risk_category,
    get_benchmark_for_category,
    calculate_simple_rolling_return,
    calculate_upside_downside_capture,
...
def main():
    ...
    # GLOBAL FILTER: Remove all Debt schemes as per user request
    filtered_df = filtered_df[~filtered_df['Category'].str.contains("Debt Scheme", case=False, na=False)]

    # GLOBAL FILTER: Remove FoF Domestic UNLESS it is a Gold or Silver fund
    # Many Gold/Silver ETFs are categorized as FoF Domestic. We want to keep those.
    # Logic: Keep if (NOT FoF Domestic) OR (Name contains Gold/Silver)
    fof_filter = ~filtered_df['Category'].str.contains("FoF Domestic", case=False, na=False)
    gold_silver_rescue = filtered_df['Scheme Name'].str.contains("Gold|Silver", case=False, na=False)
    
    filtered_df = filtered_df[fof_filter | gold_silver_rescue]
    ...
    if selected_subcats:
        ...
        if "Silver ETF" in selected_subcats:
            ...
            silver_part = filtered_df[
                (filtered_df['Scheme Name'].str.contains('Silver', case=False, na=False)) &
        ...
        if "Gold ETF" in selected_subcats:
             gold_part = filtered_df[
                (filtered_df['Scheme Name'].str.contains('Gold', case=False, na=False)) &
                (filtered_df['Category'].str.contains('ETF|FoF', case=False, na=False))
            ]
    ...
    if not filtered_df.empty:
        ...
        if st.sidebar.button("⚡ Compute Returns & Risk"):
            ...
            for i, (idx, row) in enumerate(candidates.iterrows()):
                ...
                # Pre-fetch benchmark data if available to avoid repeated internal calls
                from backend.analytics import download_benchmark
                benchmark_name = get_benchmark_for_category(row['Category'], scheme_name=row['Scheme Name'])
                ...
                res_row['Alpha'] = calculate_alpha(hist_series, benchmark_series=benchmark_series, category=row['Category'])
                ...
                res_row['Information Ratio'] = calculate_information_ratio(hist_series, benchmark_series=benchmark_series, category=row['Category'])
                res_row['Tracking Error'] = calculate_tracking_error(hist_series, benchmark_series=benchmark_series, category=row['Category'])
                ...
                if benchmark_series is not None and not benchmark_series.empty:
                    ...
                    res_row['Upside Capture'] = None
                    res_row['Downside Capture'] = None
