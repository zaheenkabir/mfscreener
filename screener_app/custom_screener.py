...
def main():
    ...
    CATEGORY_HIERARCHY = {
        "Equity": [
            ...
            "Flexi Cap Fund", "Multi Cap Fund", "Focused Fund", "Value Fund", "Contra Fund",
            "Dividend Yield Fund", "Equity Linked Savings Scheme (ELSS)", "Sectoral Fund - Banks & Financial Services",
            "Sectoral Fund - Infrastructure", "Sectoral Fund - Technology", "Sectoral Fund - Pharma & Health Care",
            "Sectoral Fund - Consumption", "Sectoral Fund - Energy & Power", "Sectoral Fund - Auto",
            "Sectoral Fund - Service Industry", "Thematic Fund", "Thematic Fund - MNC", "Thematic Fund - Global",
            "Index Fund"
        ],
        "Debt": [
            "Liquid Fund", "Overnight Fund", "Money Market Fund", "Ultra Short Duration Fund",
            "Low Duration Fund", "Short Duration Fund", "Medium Duration Fund", "Medium to Long Duration Fund",
            "Long Duration Fund", "Dynamic Bond Fund", "Corporate Bond Fund", "Credit Risk Fund",
            "Banking & PSU Fund", "Floater Fund", "Gilt Fund", "Gilt Fund with 10 year constant duration",
            "Fixed Maturity Plans", "Interval Plans"
        ],
        "Hybrid": [
            "Conservative Hybrid Fund", "Balanced Hybrid Fund", "Aggressive Hybrid Fund",
            "Dynamic Asset Allocation Fund", "Balanced Advantage Fund", "Multi Asset Allocation Fund",
            "Arbitrage Fund", "Equity Savings"
        ],
        "Commodity": [
            "Silver ETF", "Gold ETF", "FoFs - Gold", "FoFs (Domestic) - Silver"
        ],
        "Other": [
            "Index Funds - Debt Oriented", "FoFs (Domestic) - Equity Oriented", "FoFs (Overseas)",
            "FoFs (Domestic) - Debt Oriented", "FoFs (Domestic) - Hybrid", "Solution Oriented - Children's Fund",
    ...
    for broad_cat, sub_cats in CATEGORY_HIERARCHY.items():
        with st.sidebar.expander(broad_cat, expanded=False):
            ...
            selected = st.multiselect(f"Select {broad_cat} Schemes", sub_cats)
            if selected:
    ...
    if selected_subcats:
        import re
        corrected_subs = [s.replace("&", "(?:&|and)") for s in selected_subcats]
        pattern = "|".join(map(re.escape, corrected_subs))
        pattern = pattern.replace(r"\(\?\:&\ |and\)", "(?:&|and)")
