...
def calculate_alpha(series, benchmark_series=None, risk_free_rate=0.06, category=None):
    ...
    # Normalize and localize dates to ensure robust intersection (midnight-naive)
    ...
    benchmark_series.index = benchmark_series.index.normalize().tz_localize(None)
    ...
    # Calculate returns and align to remove NaNs from either
    fund_returns = fund_aligned.pct_change()
    bench_returns = bench_aligned.pct_change()
    
    combined_rets = pd.concat([fund_returns, bench_returns], axis=1).dropna()
    if len(combined_rets) < 100:
        return None
        
    fund_returns = combined_rets.iloc[:, 0]
    bench_returns = combined_rets.iloc[:, 1]
...
def calculate_tracking_error(series, benchmark_series=None, category=None):
    ...
    # Calculate difference in returns (perfectly aligned)
    return_diff = combined_rets.iloc[:, 0] - combined_rets.iloc[:, 1]
    ...
    if benchmark_series is not None and not benchmark_series.empty:
        ...
        if len(period_bench) >= 2:
            ...
            if len(common_dates) > 5:
                ...
                fund_ret = aligned_fund.pct_change()
                bench_ret = aligned_bench.pct_change()
                
                # Align to ensure identical lengths for covariance
                combined_p = pd.concat([fund_ret, bench_ret], axis=1).dropna()
                
                if len(combined_p) > 10:
                    fund_ret = combined_p.iloc[:, 0]
                    bench_ret = combined_p.iloc[:, 1]
