...
def get_benchmark_for_category(category):
    ...
    # For debt/hybrid, Alpha doesn't make sense, return None
    else:
        return None
...
def calculate_alpha(series, benchmark_series=None, risk_free_rate=0.06, category=None):
    ...
    if len(common_dates) < 252:  # Need at least 1 year of overlapping data
        return None
...
def calculate_period_metrics(fund_series, benchmark_series, years):
    ...
    if not fund_series.empty:
        fund_series.index = pd.to_datetime(fund_series.index).normalize()
